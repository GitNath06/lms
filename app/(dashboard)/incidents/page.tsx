...
export type UserRoleMode = 'super_admin' | 'lab_incharge' | 'hod' | 'faculty'

export default function IncidentsPage() {
  const { incidents } = useIncidentState()

  // Active Role Simulation (For Testing & Role Restrictions)
  const [currentRole, setCurrentRole] = useState<UserRoleMode>('lab_incharge')
  ...
  }, [incidents, labFilter, severityFilter, statusFilter, dateFilter, searchQuery])
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-5">
        ...
        {/* Role Preview Switcher & Report Action */}
        <div className="flex flex-wrap items-center gap-3">
          {/* Active Role Simulator */}
          <div
            className="flex items-center gap-2 p-1 px-3 rounded-xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 font-mono text-xs shadow-2xs"
            title="Temporary preview simulator. In the next phase, user role will automatically bind to authenticated staff profile."
          >
            <span className="px-1.5 py-0.5 rounded bg-indigo-100 dark:bg-indigo-950/80 text-indigo-700 dark:text-indigo-300 font-bold text-[9px] uppercase tracking-wider">
              Preview Mode
            </span>
            <select
              value={currentRole}
              onChange={(e) => {
                setCurrentRole(e.target.value as UserRoleMode)
                triggerToast(`Role preview switched to: ${e.target.value.replace('_', ' ').toUpperCase()}`)
              }}
              className="bg-transparent font-bold text-zinc-900 dark:text-zinc-100 border-none text-xs focus:outline-none cursor-pointer py-1"
            >
              <option value="lab_incharge">Lab In-Charge (Manager)</option>
              <option value="super_admin">Super Admin (Executive)</option>
              <option value="hod">Head of Department (HOD)</option>
              <option value="faculty">Faculty Member (Standard User)</option>
            </select>
          </div>
      ...
      <div
        className={`p-3 px-4 rounded-xl border text-xs font-mono flex flex-col sm:flex-row sm:items-center justify-between gap-2 ${
          currentRole === 'faculty'
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span>
            {currentRole === 'faculty' && (
              <>
                <strong>Faculty Read-Only View:</strong> You can file incident reports via the button above. Management controls are reserved for Lab In-Charge, Admin & HOD.
              </>
            )}
            {currentRole === 'lab_incharge' && (
              <>
                <strong>Lab In-Charge Authorized:</strong> You can mark cases <em>Under Repair</em>, <em>Replaced</em>, <em>Resolved</em>, or forward to <em>HOD</em>.
              </>
            )}
            {currentRole === 'super_admin' && (
              <>
                <strong>Super Admin Executive:</strong> Global operational authority across all 3 laboratories with HOD escalation rights.
              </>
            )}
            {currentRole === 'hod' && (
              <>
                <strong>Head of Department (HOD):</strong> Final institutional decision-maker for critical breakages and escalated disciplinary cases.
              </>
            )}
          </span>
        </div>
        ...
        <span className="text-[10px] text-zinc-400 font-mono shrink-0">
          Auto-profile binding enabled in Phase 4
        </span>
      </div>
            ...
            <table className="w-full text-left text-xs font-mono border-collapse">
              <thead>
                <tr className="border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/80 dark:bg-zinc-900/80 text-zinc-500 font-bold uppercase text-[10px]">
                  ...
                  <th className="py-2.5 px-4">Class & Faculty</th>
